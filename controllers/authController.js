...
module.exports={
    ...
    async login(req,res,next){
        try{
            ...
            if(!usuario){
                return res.render('login',{title:'Iniciar Sesion',error:'Credenciales Invalidas',old:{email}});
            }
        ...
        try {
            ...
            if (!fecha_nacimiento) {
                ...
                if (partes.length !== 3) {
                    ...
                    const birth = new Date(y, m - 1, d); // <-- local, sin lío de UTC
            ...
            // Si hay errores, renderizar el formulario con mensajes
            if (error.length) {
            ...
            req.session.usuarioAvatar   = req.session.avatarUrl || '/default-avatar.png';

            res.redirect('/');
        } catch (err) {
            if (t) await t.rollback();

            // Por si aún así la BD tira error de duplicado de email
            if (
